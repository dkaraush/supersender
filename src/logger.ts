...
global.verbose = function (...args : any[]) {
    pushLog(colors.bold('[v]'.grey) + (' ['+timedate()+'] '+callee()).grey + args.map(toString).join(' ').grey);
}
global.log = function (...args : any[]) {
    pushLog(colors.bold('[i]'.cyan) + (' ['+timedate()+'] '+callee()).cyan + args.map(toString).join(' '));
}
global.warn = function (...args : any[]) {
    pushLog(colors.bold('[!]'.yellow) + (' ['+timedate()+'] '+callee()).yellow + args.map(toString).join(' '));
}
global.error = function (...args : any[]) {
    pushLog(colors.bold('[!]'.red) + (' ['+timedate()+'] '+callee()).red + args.map(toString).join(' '));
}
global.fatal = function (...args : any[]) {
    pushLog(colors.bold('[x]'.red + (' ['+timedate()+'] '+callee()).red) + args.map(toString).join(' '));
